...
with open('lemmas.csv', 'rb') as csvfile:
...
def map_function(s):
        ...
        meta_data = meta_data.replace("<","")
        if(num(meta_data.split(".")[1]) != "no" ): # luc.1.1
            doc_id, chap , line_num = meta_data.split(".")
        elif(len(meta_data.split(".")) == 4 and not  num(meta_data.split(".")[1]) != "no"): #vergil
            doc_id_1 , doc_id_2 , chap , line_num = meta_data.split(".")
            doc_id = doc_id_1 + doc_id_2
        elif(len(meta_data.split(".")) == 3):  # ambrose
        ...
        for i in range(0,len(txt_tokens)-2):  #replace token with lemma
            ...
            if(txt_tokens[i+2] in dict):
                txt_3_lemma = dict[txt_tokens[i+2]]
                while '' in txt_3_lemma:
                    txt_3_lemma.remove('')
            else:
                txt_3_lemma = txt_tokens[i+2]
            
            
            
            
            pair = itertools.product(txt_1_lemma,txt_2_lemma,txt_3_lemma)
            for part in pair:
                txt_pair = part[0] + "," +  part[1] + "," + part[2]
...
import time
out_file_name = "output_3_gram/out" + str(time.time())
